//! Phase unwrapping. Wrapped phase lives in `(-π, π]`, but displacement is
//! continuous and can exceed one period, so we remove the 2π jumps to recover a
//! continuous phase. This is inherently sequential (each correction depends on
//! the running total), so it runs host-side rather than as a backend kernel.

use vernier_core::Real;
use vernier_core::scalar::consts::{PI, TAU};

/// Unwraps a 1D sequence of wrapped phases in place, adding/subtracting 2π
/// whenever the step between samples exceeds π.
...
/// Unwraps a 2D phase map outward from the center in four quadrants, matching
/// C++ `quartersUnwrapPhase` in `Spatial.cpp`. It propagates along the center
/// row both ways, then sweeps each column from the accumulated row offset —
/// more robust than separable row-by-row unwrapping on radially continuous maps.
